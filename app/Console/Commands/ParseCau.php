...
class ParseCau extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'Parse information from *.pdf file by CAD pair (Europe style)';
            ...
            if (count($content) !== 0) {
                foreach ($content as $month => $month_data) {
                    ...
                    if (!empty($option_by_month)) {
                        $other_month = new Cau($option_by_month->_expiration);
